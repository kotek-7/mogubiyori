...
- **ずかん**：育てたなかまと、初めて作った料理のカードを集めます。全310種から名前・材料で検索し、種類・調理時間・難しさ・獲得状態で絞り込めます。未獲得の料理も材料と手順を読んで食卓へ進めます。
...
最初のなかまを迎えたら、チュートリアルで操作を練習できます。練習は本編の報酬や記録を変更せず、途中で休んで後から再開できます。写真選択・食卓・食事は、それぞれ独立した画面です。成長や新しいカードは一つずつお祝いします。食後のXP獲得やストリーク更新は、結果を確認してボタンを押すまで表示します。
...
追加300種のレシピは、本体の「ずかん → レシピカード」に統合しています。食卓でも同じ検索一覧から料理を選べ、初めてごはんをあげるとカードと報酬を獲得します。既存カードと食事記録はそのまま残ります。

追加予定のなかま36種・帽子と背景72種は、開発サーバーの `/expansion/index.html` で成長段階と着せ替えをプレビューできます。このページの「料理カード」は本体の図鑑へ移動します。追加のなかまの加入・アイテム購入は未接続です。素材の再生成・検証と採用状況は[追加コンテンツの仕様](docs/content-expansion.md)を参照してください。
