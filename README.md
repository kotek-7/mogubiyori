...
候補は`shared/content/recipes.ts`の310種のレシピと、`shared/content/dishes.ts`の201種の料理から選びます。ごはん・麺・パンや粉もの・肉・魚介・卵や豆・野菜や副菜・汁物や鍋・おやつや果物・その他のおかずの10分類で、具体的なレシピを特定できなくても、餃子・唐揚げ・寿司・オムライスなどの名前で記録できます。「料理を選ぶ」では名前・読み・別名で検索し、種類で絞り込んで12件ずつ閲覧できます。カタログ外の料理も、写真から分かる名前・食品群・量を食事内容に反映します。種類での記録は通常の経験値と毎日の報酬を獲得し、料理カードは具体的なレシピを選んだ場合に獲得します。

認識には全レシピと全種類のカタログを1回のモデル呼び出しで渡します。名前・別名・識別に必要な説明をまとめた配列形式で、繰り返す項目名を省いて入力を抑えます。写真で確認できる範囲の料理名を選び、具材・味付けが不明ならパスタや煮物など広い種類を使います。候補の追加は対応範囲の拡張であり、実際の認識率を保証するものではありません。
