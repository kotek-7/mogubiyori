...
スクリーンショットは縦を中心に選べる構成です。完成品は `artifacts/presentation-kit/`、一括配布用は `artifacts/mogubiyori-presentation-kit.zip` にあります。`index.html` をブラウザーで開くと、ネット接続なしで日本語検索・用途や比率による絞り込み・拡大表示・動画再生ができます。

## 収録内容

| 素材                             |      数量 | 形式・サイズ                                      |
| -------------------------------- | --------: | ------------------------------------------------- |
| ゲーム画面                       |     110枚 | スマホ57枚・横長53枚。主に780×1688／1920×1080     |
| 操作動画                         |      24本 | 12操作×縦横。MP4、720×1280／1440×810、30fps、無音 |
| もぐ・料理・アイテム・背景・ロゴ | 1,021種類 | 各PNG＋SVG。669種類が背景透過                     |
...
形式違いをまとめると **1,197種類**。このほか、代表素材の一覧画像、CSV／JSON索引、撮影条件と生成プロンプトを同梱します。
...
原画素材はソースのReact SVGと配布SVGから書き出しています。現在のゲーム一覧に登録された素材と、未登録の「拡張カタログ」、以前の「旧版」を分類しています。素材全体の点数は、ゲーム内で利用できる種類数とは異なります。画面と動画は、5つのナビゲーション・記録・レポートを含む `bb8c474` 時点のアプリに揃えています。
...
110枚の画面・24本の動画の撮影成功を確認しました。全原画PNGの寸法・透明度、SVGの外部参照なし、全動画のデコードと映像形式、代表画像と動画の冒頭・末尾を確認しています。素材一覧も検索・絞り込み・拡大・動画表示を確認し、ZIPは整合性検査を通しています。
